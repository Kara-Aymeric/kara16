...
class CommissionAgent(models.Model):
    ...
    _description = "Object to manage commission generated automatically"

    salesperson_id = fields.Many2one("res.users", string="Salesperson")
    commission_rule_id = fields.Many2one("commission.salespeople.rule", string="Commission Rule")
    explanation = fields.Text(related="commission_rule_id.explanation", readonly=True)
    amount = fields.Float(string="Commission Amount")
    commission_date = fields.Date(string="Commission Date", default=lambda self: self._default_commission_date())
    ...
    def action_open_commission_details(self):
        ...
        return {
            ...
            'res_model': 'commission.details.wizard',
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'view_id': self.env.ref('commission_agent.commission_details_wizard_form_view').id,
            'target': 'new',
            'context': ctx,
        }

    # Business logic
    def get_default_rule_for_employee(self, user_id):
        employee = self.env['hr.employee'].search([("user_id", "=", user_id)])
        return employee.commission_rule_id if employee and employee.commission_rule_id else None
    ...
    def check_every_order_line(self, order, payroll_deduction, rule):
        for move in order.invoice_ids:
            ...
            rule_id = self.get_default_rule_for_employee(user_id=move.invoice_user_id.id)
            ...
            if commission_calculated and last_date_of_month:
                ...
                if not calcul:
                    ...
                    if has_follower_technician:
                        ...
                        rule_id = self.get_default_rule_for_employee(user_id=move.technician_id.id)
                        if rule_id is not None:
    ...
    def check_every_order(self, count, order, payroll_deduction, rule):
        ...
        rule_id = self.get_default_rule_for_employee(user_id=order.user_id.id)
        vals = {
        ...
        if vals and order.user_id:
            ...
            if has_follower_technician:
                ...
                rule_id = self.get_default_rule_for_employee(user_id=order.support_technician_id.id)
                if rule_id is not None:
                    is_already_done = self.env["commission.calcul"].search_count([
                        ('order_id', '=', order.id),
                        ('commission_rule_id', '=', rule_id.id)
