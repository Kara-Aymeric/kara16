...
class AgentCommission(models.Model):
    ...
    discount_percentage_value = fields.Float(
        string="Percentage value (discount)", help="Value to calculate the discount commission on the product",
        tracking=True
    )
    ...
    # discount_sale_commission = fields.Float(
    #     string="Discount commission", store=True, compute="_compute_discount_sale_commission", digits=(12, 2),
    #     help="Final value of the discount commission calculated automatically", tracking=True
    # )
    ...
    # @api.depends('discount_percentage_value', 'product_template_id.list_price')
    # def _compute_discount_sale_commission(self):
    #     for record in self:
    #         if record.discount_percentage_value > 0:
    #             record.discount_sale_commission = (record.discount_percentage_value/100) * \
    #                                               record.product_template_id.list_price
    ...
    @api.onchange('discount_percentage_value')
    def _onchange_discount_percentage_value(self):
        if not 0 <= self.discount_percentage_value <= 100:
            raise ValidationError("The discount percentage value must be between 0 and 100")
